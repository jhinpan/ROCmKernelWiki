---
id: pr-composable_kernel-3287
repo: ROCm/composable_kernel
pr: 3287
title: '[CK_TILE] Optimize Flatmm MXFP4 by Eliminating Runtime Division by 2'
author: DDEle
date: '2025-12-08'
url: https://github.com/ROCm/composable_kernel/pull/3287
source_category: upstream-code
architectures:
- gfx942
tags:
- fp4
- gfx942
- mxfp
techniques:
- software-pipelining
hardware_features:
- fp4
- mxfp
kernel_types: []
languages:
- composable-kernel
- hip
- cpp
captured_at: '2026-05-15'
status: merged
merge_sha: 878b4e7f46d7
inclusion_reason: kernel path 'include/ck_tile/ops/flatmm/kernel/mx_flatmm_kernel.hpp';
  keyword 'fp4'
changed_paths:
- include/ck_tile/ops/flatmm/kernel/mx_flatmm_kernel.hpp
- include/ck_tile/ops/flatmm/pipeline/mx_flatmm_pipeline_agmem_bgmem_creg_v1.hpp
- include/ck_tile/ops/flatmm/pipeline/mx_flatmm_pipeline_agmem_bgmem_creg_v1_policy.hpp
facet_source: inferred
---

# [CK_TILE] Optimize Flatmm MXFP4 by Eliminating Runtime Division by 2

**Repository:** [ROCm/composable_kernel](https://github.com/ROCm/composable_kernel) · **PR:** [#3287](https://github.com/ROCm/composable_kernel/pull/3287) · **Merged:** 2025-12-08 · **Author:** @DDEle

**Inclusion reason:** kernel path 'include/ck_tile/ops/flatmm/kernel/mx_flatmm_kernel.hpp'; keyword 'fp4'

## Summary (from upstream PR description)

Proposed changes
As title. Dependencies:

#3272
#3285

Checklist
Please put an x into the boxes that apply. You can also fill these out after creating the PR. If you're not sure, please don't hesitate to ask.

 I have added tests relevant to the introduced functionality, and the unit tests are passing locally
 I have added the test to REGRESSION_TESTS list defined at the top of CMakeLists.txt in tests/CMakeLists.txt, IF the test takes more than 30 seconds to run.
 I have added inline documentation which enables the maintainers with understanding the motivation
 I have removed the stale documentation which is no longer relevant after this pull request
 (If this change is user-facing) I have added release notes which provide the end users with a brief summary of the improvement from this pull request
 I have run clang-format on all changed files
 Any dependent changes have been merged

## Changed files (3 total, first 3 shown)

- `include/ck_tile/ops/flatmm/kernel/mx_flatmm_kernel.hpp`
- `include/ck_tile/ops/flatmm/pipeline/mx_flatmm_pipeline_agmem_bgmem_creg_v1.hpp`
- `include/ck_tile/ops/flatmm/pipeline/mx_flatmm_pipeline_agmem_bgmem_creg_v1_policy.hpp`

## Provenance

- Merge commit: `878b4e7f46d7`
- Captured at knowledge cutoff: 2026-05-15
- Source of truth: the upstream PR linked above. This page summarizes upstream
  metadata; consult the PR for the authoritative diff.
