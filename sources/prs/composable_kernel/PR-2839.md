---
id: pr-composable_kernel-2839
repo: ROCm/composable_kernel
pr: 2839
title: '[CK TILE GEMM] Fixed permutation error in shuffle_b_permuteN'
author: CongMa13
date: '2025-09-14'
url: https://github.com/ROCm/composable_kernel/pull/2839
source_category: upstream-code
architectures:
- gfx942
tags:
- gemm
- gfx942
techniques:
- epilogue-fusion
- preshuffle-layout
hardware_features: []
kernel_types:
- gemm
languages:
- composable-kernel
- hip
- cpp
captured_at: '2026-05-15'
status: merged
merge_sha: e5d73da2da96
inclusion_reason: kernel path 'example/ck_tile/03_gemm/gemm_weight_preshuffle_invoker.hpp';
  keyword 'gemm'
changed_paths:
- example/ck_tile/03_gemm/gemm_weight_preshuffle_invoker.hpp
- include/ck_tile/ops/epilogue/cshuffle_epilogue.hpp
facet_source: inferred
related:
- kernel-flydsl-preshuffle-gemm
- technique-stream-k
- technique-split-k
---
# [CK TILE GEMM] Fixed permutation error in shuffle_b_permuteN

**Repository:** [ROCm/composable_kernel](https://github.com/ROCm/composable_kernel) · **PR:** [#2839](https://github.com/ROCm/composable_kernel/pull/2839) · **Merged:** 2025-09-14 · **Author:** @CongMa13

**Inclusion reason:** kernel path 'example/ck_tile/03_gemm/gemm_weight_preshuffle_invoker.hpp'; keyword 'gemm'

## Summary (from upstream PR description)

Proposed changes
The view dimensions in shuffle_b_permuteN function should be ordered from fastest-changing to slowest-changing dimension.
So reordered dimensions of the view to fix a numerical error of tile_example_gemm_weight_preshuffle.
Checklist
Please put an x into the boxes that apply. You can also fill these out after creating the PR. If you're not sure, please don't hesitate to ask.

 I have added tests relevant to the introduced functionality, and the unit tests are passing locally
 I have added the test to REGRESSION_TESTS list defined at the top of CMakeLists.txt in tests/CMakeLists.txt, IF the test takes more than 30 seconds to run.
 I have added inline documentation which enables the maintainers with understanding the motivation
 I have removed the stale documentation which is no longer relevant after this pull request
 (If this change is user-facing) I have added release notes which provide the end users with a brief summary of the improvement from this pull request
 I have run clang-format on all changed files
 Any dependent changes have been merged

Discussion
If this is a relatively large or complex change, feel free to start a discussion by explaining why you chos

## Changed files (2 total, first 2 shown)

- `example/ck_tile/03_gemm/gemm_weight_preshuffle_invoker.hpp`
- `include/ck_tile/ops/epilogue/cshuffle_epilogue.hpp`

## Provenance

- Merge commit: `e5d73da2da96`
- Captured at knowledge cutoff: 2026-05-15
- Source of truth: the upstream PR linked above. This page summarizes upstream
  metadata; consult the PR for the authoritative diff.
