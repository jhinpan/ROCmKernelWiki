---
id: pr-composable_kernel-3119
repo: ROCm/composable_kernel
pr: 3119
title: 'test(grouped_gemm): add unit tests for grouped_gemm bquant with preshuffleB
  true'
author: AviralGoelAMD
date: '2025-10-31'
url: https://github.com/ROCm/composable_kernel/pull/3119
source_category: upstream-code
architectures:
- gfx942
tags:
- gemm
- gfx942
- grouped-gemm
- quantization
techniques: []
hardware_features: []
kernel_types:
- grouped-gemm
- gemm
- quantization
languages:
- composable-kernel
- hip
- cpp
captured_at: '2026-05-15'
artifact_dir: artifacts/prs/composable_kernel/PR-3119
status: merged
merge_sha: 8f1274d9b655
inclusion_reason: kernel path 'example/ck_tile/17_grouped_gemm/quant_grouped_gemm.cpp';
  keyword 'gemm'
changed_paths:
- example/ck_tile/17_grouped_gemm/quant_grouped_gemm.cpp
- example/ck_tile/17_grouped_gemm/quant_grouped_gemm.hpp
- example/ck_tile/17_grouped_gemm/quant_run_grouped_gemm_example.inc
- include/ck_tile/ops/gemm/pipeline/wp_pipeline_agmem_bgmem_creg_v2.hpp
- include/ck_tile/ops/gemm_quant/kernel/gemm_quant_kernel.hpp
- include/ck_tile/ops/gemm_quant/kernel/grouped_gemm_quant_kernel.hpp
- include/ck_tile/ops/gemm_quant/pipeline/gemm_wp_bquant_pipeline_ag_bg_cr_v2.hpp
- test/ck_tile/grouped_gemm_quant/CMakeLists.txt
- test/ck_tile/grouped_gemm_quant/test_grouped_gemm_quant.cpp
- test/ck_tile/grouped_gemm_quant/test_grouped_gemm_quant_bquant.cpp
- test/ck_tile/grouped_gemm_quant/test_grouped_gemm_quant_rowcol.cpp
- test/ck_tile/grouped_gemm_quant/test_grouped_gemm_quant_tensor.cpp
- test/ck_tile/grouped_gemm_quant/test_grouped_gemm_quant_ut_cases.inc
- test/ck_tile/grouped_gemm_quant/test_grouped_gemm_util_quant.hpp
---

# test(grouped_gemm): add unit tests for grouped_gemm bquant with preshuffleB true

**Repository:** [ROCm/composable_kernel](https://github.com/ROCm/composable_kernel) · **PR:** [#3119](https://github.com/ROCm/composable_kernel/pull/3119) · **Merged:** 2025-10-31 · **Author:** @AviralGoelAMD

**Inclusion reason:** kernel path 'example/ck_tile/17_grouped_gemm/quant_grouped_gemm.cpp'; keyword 'gemm'

## Summary (from upstream PR description)

Proposed changes
Unit tests are added to maintain the integrity of work done in #3115.
We also split the list of unit tests for grouped_gemm_quant into three cpp files (filter by quant_mode) to reduce compilation time.
Checklist
Please put an x into the boxes that apply. You can also fill these out after creating the PR. If you're not sure, please don't hesitate to ask.

 I have added tests relevant to the introduced functionality, and the unit tests are passing locally
 I have added the test to REGRESSION_TESTS list defined at the top of CMakeLists.txt in tests/CMakeLists.txt, IF the test takes more than 30 seconds to run.
 I have added inline documentation which enables the maintainers with understanding the motivation
 I have removed the stale documentation which is no longer relevant after this pull request
 (If this change is user-facing) I have added release notes which provide the end users with a brief summary of the improvement from this pull request
 I have run clang-format on all changed files
 This PR is dependent on #3115.

Discussion
If this is a relatively large or complex change, feel free to start a discussion by explaining why you chose the solution you did and wh

## Changed files (14 total, first 14 shown)

- `example/ck_tile/17_grouped_gemm/quant_grouped_gemm.cpp`
- `example/ck_tile/17_grouped_gemm/quant_grouped_gemm.hpp`
- `example/ck_tile/17_grouped_gemm/quant_run_grouped_gemm_example.inc`
- `include/ck_tile/ops/gemm/pipeline/wp_pipeline_agmem_bgmem_creg_v2.hpp`
- `include/ck_tile/ops/gemm_quant/kernel/gemm_quant_kernel.hpp`
- `include/ck_tile/ops/gemm_quant/kernel/grouped_gemm_quant_kernel.hpp`
- `include/ck_tile/ops/gemm_quant/pipeline/gemm_wp_bquant_pipeline_ag_bg_cr_v2.hpp`
- `test/ck_tile/grouped_gemm_quant/CMakeLists.txt`
- `test/ck_tile/grouped_gemm_quant/test_grouped_gemm_quant.cpp`
- `test/ck_tile/grouped_gemm_quant/test_grouped_gemm_quant_bquant.cpp`
- `test/ck_tile/grouped_gemm_quant/test_grouped_gemm_quant_rowcol.cpp`
- `test/ck_tile/grouped_gemm_quant/test_grouped_gemm_quant_tensor.cpp`
- `test/ck_tile/grouped_gemm_quant/test_grouped_gemm_quant_ut_cases.inc`
- `test/ck_tile/grouped_gemm_quant/test_grouped_gemm_util_quant.hpp`

## Provenance

- Merge commit: `8f1274d9b655`
- Captured at knowledge cutoff: 2026-05-15
- Source of truth: the upstream PR linked above. This page summarizes upstream
  metadata; consult the PR for the authoritative diff.
